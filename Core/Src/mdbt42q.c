...
void ble_module_init(void)
{
  ...
  if (p == NULL) 
  {
    ...
    HAL_Delay(100);
    HAL_UART_Transmit(&huart2, (uint8_t *)"AT+RESET\r", sizeof("AT+RESET\r"), 1000);
    HAL_Delay(100);
  }
  ble_enable();
}
...
void ble_disable(void)
{
  ...
  HAL_GPIO_WritePin(GPIOC, GPIO_PIN_1, GPIO_PIN_RESET); // BLE_UART2_PD(POWER DOWN) High : Power Down
  HAL_GPIO_WritePin(GPIOC, GPIO_PIN_0, GPIO_PIN_RESET); // BLE_WAKEUP Low : Disable
  HAL_UART_Transmit(&huart2, (uint8_t *)"AT+SLEEP\r", sizeof("AT+SLEEP\r"), 1000);
  HAL_Delay(100);
}
